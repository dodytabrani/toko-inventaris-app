document.addEventListener('DOMContentLoaded', async () => {
    ...
    // --- ELEMEN BARU UNTUK PENCARIAN DAN PAGINASI ---
    const searchInput = document.getElementById('searchItemInput'); // ID sesuai HTML Anda
    const unitTypeFilter = document.getElementById('filterUnitType'); // ID sesuai HTML Anda
    const prevPageBtn = document.getElementById('prevPageBtn');
    const nextPageBtn = document.getElementById('nextPageBtn');
    const pageNumbersDiv = document.getElementById('pageNumbers');
    // Pastikan Anda sudah menambahkan elemen-elemen ini di `index.html`
    ...
    // Hapus event listener DOMContentLoaded ini karena sudah ada di baris paling atas
    // document.addEventListener('DOMContentLoaded', async () => { ... });
    ...
    async function checkUserRoleAndRedirect(user) {
        if (!user) {
            ...
            // Menampilkan bagian otentikasi jika user null
            authSection.style.display = 'block';
            appSection.style.display = 'none';
            adminPanelSection.style.display = 'none';
    ...
    async function showUserApp() {
        ...
        // --- PENTING: Panggil loadInventarisData dengan parameter kosong untuk pencarian awal ---
    ...
    // --- Modifikasi Fungsi loadInventarisData untuk Pencarian dan Paginasi ---
    async function loadInventarisData(searchTerm = '', filterUnitType = '') {
        // Menggunakan inventarisDataDiv karena di skrip Anda, data inventaris ditempelkan ke sini
        if (!inventarisDataDiv) {
            console.error('inventarisDataDiv element not found!');
            return;
        }

        // Tampilkan pesan loading di dalam inventarisDataDiv
        ...
        query = query.eq('user_id', user.id); // Filter berdasarkan user_id
        ...
        // Terapkan filter unit type
        if (filterUnitType) {
            query = query.eq('unit_type', filterUnitType);
        }

        // Paginasi: Ambil total count terlebih dahulu
        const { count, error: countError } = await query.range(0, 0); // Ambil count tanpa mengambil data
        if (countError) {
        ...
        // Terapkan paginasi ke query utama
        ...
        if (error) {
            ...
            if (inventories.length === 0) {
                ...
                `;
                // Pastikan Anda juga membuat elemen tbody dengan ID inventarisTableBody di HTML
                // jika belum ada, atau sesuaikan reference `inventarisTableBody` di sini.
                // Saya telah mengubahnya untuk menggunakan id "inventarisTableBody"
                // pada elemen tbody yang akan dibuat secara dinamis di sini.
                ...
                // Setelah HTML dirender, update kontrol paginasi
                updatePaginationControls();
            }
        }
    }

    // --- Event Listener Delegasi (TIDAK ADA PERUBAHAN DI SINI UNTUK FUNGSI INI) ---
    // Ini adalah blok kode yang sudah Anda kirimkan, saya tidak mengubahnya.
    // Hanya memastikan `loadInventarisData()` dipanggil dengan parameter yang benar setelah edit/delete.
        ...
        if (e.target.matches('.delete-item-btn')) {
            ...
            if (confirm('Apakah Anda yakin ingin menghapus item ini?')) {
                ...
                if (deleteError) {
                    ...
                    // --- PENTING: Panggil dengan parameter pencarian/filter saat ini ---
                    await loadInventarisData(searchInput.value.trim(), unitTypeFilter.value);
                }
            }
        }
    });
    ...
    // --- EVENT LISTENERS BARU UNTUK PENCARIAN DAN FILTER ---
    if (searchInput) {
        searchInput.addEventListener('keyup', () => {
            const searchTerm = searchInput.value.trim();
            const filterUnitTypeVal = unitTypeFilter ? unitTypeFilter.value : '';
            currentPage = 1; // Reset ke halaman pertama saat pencarian/filter baru
            loadInventarisData(searchTerm, filterUnitTypeVal);
        });
    } else {
        console.error('ERROR: Elemen searchInput (ID: searchItemInput) tidak ditemukan! Pastikan ada di HTML.');
    }
    ...
    if (unitTypeFilter) {
        ...
        console.error('ERROR: Elemen unitTypeFilter (ID: filterUnitType) tidak ditemukan! Pastikan ada di HTML.');
    }

    // --- EVENT LISTENERS BARU UNTUK PAGINASI ---
    if (prevPageBtn) {
        ...
        console.error('ERROR: Elemen prevPageBtn (ID: prevPageBtn) tidak ditemukan! Pastikan ada di HTML.');
    }
